...
F7 = 36*D - 11*N
assert sp.expand(RPD - (D*Rp + F7*U - 5*U**2)).subs({Rp:D*P-K*U, U:D*K-N}) == 0

Lperp = (55*D-18*N)*(alpha-T*K+3*T) + N*T
Fperp = 53 - 15*K
rhs_perp = (55*D-18*N)*alpha + 3*T*Rp + T*Fperp*U
expr = sp.expand(Lperp-rhs_perp).subs({Rp:D*P-K*U, U:D*K-N})
assert sp.expand(expr) == 0

LD3 = T*(55*D-18*N) - 6*N*(alpha-T*K+3*T)
Fstar = 5*K - 36
rhs_par = T*Rp - T*Fstar*U - 6*N*alpha
expr = sp.expand(LD3-rhs_par).subs({Rp:D*P-K*U, U:D*K-N})
assert sp.expand(expr) == 0
...
# Normalized second-layer condition: D*R1+(F7/7)*U0=0 -> 2R1+U0=0 after removing N.
# D/N=4 mod7 and (F7/7)/N=2 mod7; divide by 2N.
assert d2 % 7 == 4
assert (2 * pow(4,-1,7)) % 7 == 4  # sanity only
