...
floor_majorant_M11 = F(2**19, 5**22)  # 2^(M+8)/5^(2M), M=11
...
# a_Delta initially has 2^((eta-M)/2) 5^(3M+2eta-4d)/(4 c_Q^3).
# c_Q^3 contributes 2^(3eta+6) 5^(3eta+3-3d), and the explicit 4
# contributes another 2^2. Thus the remaining exponents are:
# 2^(-(M+5eta)/2-8), 5^(3M-d-eta-3).
# We certify those linear identities coefficient-wise.
# twice the 2-exponent: eta-M - 2*(3eta+8) = -M-5eta-16.
assert (1 - 6) == -5
assert -1 == -1
assert -16 == -16
...
print("OK: A2 CRT quotient canonically separates the two Gaussian high-factor sides")
